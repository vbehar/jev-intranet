...
# domain classes
%w(user group subscription workflow_action payment post event participation).each do |f|
  require(File.join(File.dirname(__FILE__), 'domain', f))
end

# controllers (sinatra routes)
%w(helper main registration account subscriptions users posts events admin api_v1).each do |f|
